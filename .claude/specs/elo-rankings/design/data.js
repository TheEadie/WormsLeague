...
// ── ELO ratings ───────────────────────────────────────────────────────────
// Finishing positions: winner first, then remaining players in their original
// `players` array order. Deterministic from existing data.
window.gamePositions = (g) => [g.winner, ...g.players.filter(p => p !== g.winner)];
...
// Multi-player FFA Elo. For each pair, the higher-finishing player "wins".
// Pairwise deltas are scaled by 1/(n-1) so a single match doesn't over-swing.
window.computeLeagueElo = (() => {
  const cache = {};
  const SEED = 1000;
  const K = 32;
  return (leagueId) => {
    if (cache[leagueId]) return cache[leagueId];
    const league = window.LEAGUE_DEFS[leagueId];
    const ratings = {};
    const history = {};
    league.playerIds.forEach(p => { ratings[p] = SEED; history[p] = [{ gameId: null, after: SEED, delta: 0 }]; });

    // Oldest first so deltas accumulate chronologically.
    const games = window.gamesForLeague(leagueId).slice()
    ...
    games.forEach(g => {
      ...
      const rounded = {};
      Object.entries(acc).forEach(([p, d]) => {
        const r = Math.round(d);
        rounded[p] = r;
        ratings[p] += r;
        history[p].push({ gameId: g.id, after: ratings[p], delta: r });
      });
      deltas[g.id] = rounded;
    });

    const peak = {};
    Object.entries(history).forEach(([p, h]) => { peak[p] = Math.max(...h.map(x => x.after)); });

    const standings = league.playerIds
      .map(p => {
        const h = history[p];
        const last5 = h.slice(-6).slice(1).map(x => x.delta);
        return {
          playerId: p,
          rating: ratings[p],
          peak: peak[p],
          ...
          history: h,
          last5,
          lastDelta: h[h.length - 1]?.delta || 0,
        };
      })
      .sort((a, b) => b.rating - a.rating);

    cache[leagueId] = { ratings, deltas, standings, gameCount: games.length };
