...
function DetailHero({ game, summary }) {
  ...
  const positions = window.gamePositions(game);
  const { deltas, ratings } = window.computeLeagueElo(game.leagueId);
  const gameDelta = deltas[game.id] || {};
  ...
  return (
    <Paper variant="outlined" sx={{ p: 3, mb: 2 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2, flexWrap: 'wrap' }}>
        ...
        <Chip
          label={`♛ ${window.PLAYERS[game.winner].name} won  ·  +${gameDelta[game.winner] ?? 0} ELO`}
          color="warning"
          size="small"
      ...
      <Stack direction="row" spacing={1} mt={0.5} mb={2.5} flexWrap="wrap" useFlexGap>
        {positions.map((pid, i) => {
          const p     = window.PLAYERS[pid];
          const place = i + 1;
          const d     = gameDelta[pid] ?? 0;
          const isWin = place === 1;
          const medal = ['#ffca28','#bdbdbd','#cd7f32'][i] || 'rgba(255,255,255,0.18)';
          return (
            <Paper
              key={pid}
              variant={isWin ? 'elevation' : 'outlined'}
              elevation={isWin ? 3 : 0}
              sx={{
                display: 'flex', alignItems: 'center', gap: 1.25,
                pl: 0.5, pr: 2, py: 0.75, borderRadius: 99,
                ...(isWin
                  ? { border: '1px solid', borderColor: 'warning.main', bgcolor: 'warning.main' }
                  : {}),
              }}
            >
              <Box sx={{
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', lineHeight: 1.1 }}>
                <Typography fontWeight={700} fontSize={13} color={isWin ? '#000' : 'text.primary'}>
                  {p.name}
                </Typography>
                <Typography fontSize={10} color={isWin ? 'rgba(0,0,0,0.65)' : 'text.secondary'}>
                  {p.team}
                </Typography>
              </Box>
              <Box sx={{
                display: 'flex', flexDirection: 'column', alignItems: 'flex-end',
                ml: 1, lineHeight: 1.1, pl: 1,
                borderLeft: '1px solid',
                borderColor: isWin ? 'rgba(0,0,0,0.2)' : 'divider',
              }}>
                <Typography sx={{
                  fontFamily: 'JetBrains Mono', fontSize: 13, fontWeight: 700,
                  color: isWin ? '#000' : 'text.primary',
                }}>
                  {ratings[pid]}
                </Typography>
                <Typography sx={{
                  fontFamily: 'JetBrains Mono', fontSize: 10, fontWeight: 700,
                  color: isWin ? 'rgba(0,0,0,0.65)'
                       : d > 0 ? 'success.main'
                       : d < 0 ? 'error.main' : 'text.disabled',
                }}>
                  {d > 0 ? '+' : ''}{d} ELO
                </Typography>
              </Box>
