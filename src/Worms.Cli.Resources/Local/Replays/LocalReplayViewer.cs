    ...
    : IResourceViewer<LocalReplay, LocalReplayViewParameters>
{
    public async Task View(LocalReplay resource, LocalReplayViewParameters parameters)
    {
        if (parameters.Turn != default)
        {
            ...
            await wormsArmageddon.PlayReplay(resource.Paths.WAgamePath, startTime).ConfigureAwait(false);
            return;
        }

        await wormsArmageddon.PlayReplay(resource.Paths.WAgamePath).ConfigureAwait(false);
    }
}
