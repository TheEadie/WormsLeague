...
namespace Worms.Cli.Resources.Remote.Auth
{
    public class DeviceCodeLoginService : ILoginService
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            request.AddParameter("application/x-www-form-urlencoded", $"client_id={ClientId}&scope={scopes}&audience={Audience}", ParameterType.RequestBody);
            var response = await client.PostAsync(request, cancellationToken);
            
            if (response.IsSuccessful) return JsonSerializer.Deserialize<DeviceAuthorizationResponse>(response.Content!);
            ...
            CancellationToken cancellationToken)
        {
            ...
            while (!cancellationTokenSource.Token.IsCancellationRequested)
            {
                ...
                request.AddHeader("content-type", "application/x-www-form-urlencoded");
                request.AddParameter("application/x-www-form-urlencoded", $"grant_type=urn%3Aietf%3Aparams%3Aoauth%3Agrant-type%3Adevice_code&device_code={deviceCodeResponse.DeviceCode}&client_id={ClientId}", ParameterType.RequestBody);
                var response = await client.ExecutePostAsync(request, cancellationTokenSource.Token);
                
                if (response.IsSuccessful) return JsonSerializer.Deserialize<TokenResponse>(response.Content!);
